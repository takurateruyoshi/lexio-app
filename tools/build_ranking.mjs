...
// practice / spectate は除外。同一牌譜の重複（再送）は at+round で除去する。
// 出力: { v, updatedAt, rounds, entries: [{ d, n, best, at, rounds }] } 上位100件。
// collector_headless からは buildRanking() をライブ集計のシード生成に利用する。
"use strict";
...
// recordsDir 配下の全JSONLを集計してランキングオブジェクトを返す
export function buildRanking(recordsDir) {
  const byDev = new Map();   // d -> { d, n, nAt, best, at, rounds }
  const seen = new Set();
  let rounds = 0;

  for (const file of walkJsonl(recordsDir)) {
  ...
  return { v: 1, updatedAt: new Date().toISOString(), rounds, entries };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  console.log(`ranking: ${result.entries.length} devices from ${result.rounds} rounds -> ${OUT}`);
}
